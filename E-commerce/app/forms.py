...
AuthenticationForm, UsernameField,PasswordChangeForm,SetPasswordForm)
...
class MyPasswordReserForm(PasswordResetForm):
    email = forms.EmailField(label=_('Email'),max_length=154,widget=forms.EmailInput(attrs=
    {'autocomplete':'email','class':'form-control'}))

class MySetPasswordForm(SetPasswordForm):
    new_password1 = forms.CharField(label=_('New Password'),strip=False,widget=forms.PasswordInput(attrs=
    {'auto-complete':'new-password','class':'form-control'}),help_text=password_validation.password_validators_help_text_html())
